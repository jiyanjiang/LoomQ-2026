#!/usr/bin/env python3
"""
本源量子云真机实验：GHZ state（3 比特三体纠缠）· 本源悟空 180

实验配置（用户 2026-08-20 指定）：
  - 电路  : GHZ state (H q0; CNOT q0->q1; CNOT q0->q2)，3 量子比特
            |GHZ> = (|000> + |111>) / sqrt(2)，预期 000≈50% / 111≈50%
...
继承自 bellstate v2（2026-08-20 已验证成功，任务 838F10281C75D8756F10394B1B840B09）的修正：
  - 转换器 : starter_kit/adapter.py::_qasm2_to_originir（契约格式逐比特测量
            MEASURE q[i], c[i]），弃用 pyqpanda convert_qasm_string_to_originir
            （其输出整寄存器 MEASURE q,c 报 "not measure node"）
  - 状态码 : pyqpanda TaskStatus 1=WAITING 2=COMPUTING 3=FINISHED 4=FAILED

设计：提交 / 轮询 两步分离（真机排队可达小时级，避免单次命令阻塞超时）：
  python scripts/originq_ghz_wukong180.py --mode submit   # 提交并落盘 task.json
  python scripts/originq_ghz_wukong180.py --mode poll     # 查询任务状态，完成则落盘结果
  python scripts/originq_ghz_wukong180.py --mode run      # submit + 自动轮询直到完成（默认）

产物（results/originq_20260820/ghz_wukong180_shots8192/）：
...
OUTPUT_DIR = os.path.join(
    ...
    f"{EXPERIMENT}_wukong180_shots{0}",  # shots 由下方真实值覆盖
)
...
def ghz_qasm() -> str:
    """GHZ state (3 比特) 的 OpenQASM 2.0 文本（存档用）。"""
    lines = [
        "OPENQASM 2.0;",
        'include "qelib1.inc";',
        "qreg q[3];",
...
def build_ghz_originir() -> str:
    """GHZ state 的 OriginIR 字符串（本源中间表示，提交载体）。

    用 QASM → OriginIR 转换而非 QProg 直传（pyqpanda 对 QProg 序列化会段错误）；
    转换器用项目已验证的 starter_kit/adapter.py::_qasm2_to_originir，
    输出契约格式逐比特测量 `MEASURE q[i], c[i]`（弃用 pyqpanda 转换器，其
    输出整寄存器 `MEASURE q,c` 司南系统解析器不认，报 "not measure node"）。
    """
    sys.path.insert(0, os.path.join(PROJECT_ROOT, "starter_kit"))
    from adapter import _qasm2_to_originir  # 仅标准库依赖，可独立 import
    return _qasm2_to_originir(ghz_qasm())
...
def cmd_submit(args, token):
    ...
    ir = build_ghz_originir()
    print(f"[2/4] OriginIR 构建完成: H q0; CNOT q0,q1; CNOT q0,q2; Measure x3")
    print(f"      {ir.strip()}")
...
def cmd_poll(args, token, max_wait_s=None):
    ...
    while True:
        ...
        if max_wait_s and (time.time() - t0) > max_wait_s:
            ...
            print(f"   稍后重跑本命令即可续查: python scripts/originq_ghz_wukong180.py --mode poll")
            return False
...
def _finalize(task, result):
    ...
    n_other = sum(v for k, v in counts.items() if k not in ("000", "111"))
    n_total = sum(counts.values())
    ghz_ratio = (n_000 + n_111) / n_total if n_total else None
    ...
    if ghz_ratio is not None and ghz_ratio >= 0.9:
        checks.append(("GHZ 保真度", True, f"{ghz_ratio:.4%} >= 90%"))
    else:
        ok = False
        checks.append(("GHZ 保真度", False, f"{ghz_ratio:.4%} < 90%"))
    if n_other == 0:
        checks.append(("零噪声", True, f"其余位串计数 = 0"))
    else:
        ok = False
        checks.append(("零噪声", False, f"存在噪声位串（非 000/111 计数 = {n_other}）"))
...
def main():
    ap = argparse.ArgumentParser(description="本源悟空 180 真机 GHZ state 实验")
    ...
    ap.add_argument("--task-name", default="LoomQ_ghz_s8192",
    ...
    OUTPUT_DIR = os.path.join(
        PROJECT_ROOT, "results", "originq_20260820",
        f"{EXPERIMENT}_wukong180_shots{args.shots}",
    )
